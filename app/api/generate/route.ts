...
export async function GET(request: Request) {
  ...
  const { textStream } = await streamText({
    ...
    <Writing Guidelines>
    - Write as if you are writing a wikipedia page, but never include external sources or references, only link to other pages in the library of everything.
    - Use markdown for formatting the page and including links.
    - Dont mention anything about how you are writing the page, just write the page.
    - Include lots of links to other pages in the library of everything.
    - Label links with a useful short description of the link, not the url it's self.
    - Dont include any other text than the page content, no introduction, no conclusion, no nothing.
    </Writing Guidelines>
    `,
  });
